...
- API 目标只能是没有查询参数、片段和路径越级段的相对路径，或支付宝 Gateway 方法。
- 自定义端点只能是没有用户信息、查询参数、片段和路径越级段的 HTTPS 基址。
- Token、`Host`、`Content-Length`、`Content-Type` 和平台签名请求头由通道独占。
- 请求签名使用平台协议规定的实际字节或 multipart 元数据；支付响应在解析前验签。
- 派生资源只能从可信 `Response` 创建，并校验通道、HTTPS 与网络地址策略。
- 下载先暂存并完成 JSON/XML 错误识别、验签和摘要校验，再复制到目标流。
