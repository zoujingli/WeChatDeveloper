...
/** 使用 JSON 文件和 `flock` 提供单机多进程刷新锁的缓存实现。 */
final class FileCacheStore implements StoreCacheInterface
{
    ...
    {
        if ($this->directory === '') {
            throw new SdkException('文件缓存目录不能为空');
        }
        if (!is_dir($this->directory) && @mkdir($this->directory, self::DIRECTORY_MODE, true) !== true) {
            throw new SdkException('无法创建文件缓存目录: ' . $this->directory);
        }
        if (!is_writable($this->directory)) {
            throw new SdkException('文件缓存目录不可写: ' . $this->directory);
        }
    }

    /** 文件不存在、过期或 JSON 无效时返回默认值。 */
    ...
    public function set(string $key, mixed $value, int $ttl): void
    {
        ...
        if (!is_dir($dir) && @mkdir($dir, self::DIRECTORY_MODE, true) !== true) {
            throw new SdkException('无法创建文件缓存子目录: ' . $dir);
        }
        ...
        if (@file_put_contents($tmp, $body, LOCK_EX) === false) {
            @unlink($tmp);
            throw new SdkException('写入文件缓存失败: ' . $tmp);
        }
        if (DIRECTORY_SEPARATOR !== '\\' && !@chmod($tmp, self::FILE_MODE)) {
            @unlink($tmp);
            throw new SdkException('无法收紧文件缓存权限: ' . $tmp);
        }
        if (!@rename($tmp, $path)) {
            @unlink($path);
            if (!@rename($tmp, $path)) {
                @unlink($tmp);
                throw new SdkException('提交文件缓存失败: ' . $path);
            }
        }
    }
    ...
    /** 使用 `flock` 在单机进程间互斥执行回调。 */
    public function lock(string $key, int $ttl, callable $callback): mixed
    {
        $path = $this->pathFor('lock:' . $key) . '.lock';
        $dir = dirname($path);
        if (!is_dir($dir) && @mkdir($dir, self::DIRECTORY_MODE, true) !== true) {
            throw new SdkException('无法创建文件缓存锁目录: ' . $dir);
        }

        $handle = @fopen($path, 'c');
        if ($handle === false) {
            throw new SdkException('无法创建文件缓存锁文件: ' . $path);
        }

        try {
            if (!flock($handle, LOCK_EX)) {
                throw new SdkException('获取文件缓存锁失败: ' . $key);
            }
